...
`)
	...
	if cfg.StalwartEnabled {
		...
		cmds = append(cmds, fmt.Sprintf(
			`STALWART_URL="http://localhost:8080" STALWART_USER="admin" STALWART_PASSWORD="%[1]s" /apps/bin/stalwart-cli apply --file /configs/stalwart/bootstrap_plan.ndjson 2>&1 || echo "[bootstrap] Stalwart bootstrap plan may have already been applied"`, cfg.StalwartAdminPassword))
		cmds = append(cmds, fmt.Sprintf(
			`STALWART_URL="http://localhost:8080" STALWART_USER="admin" STALWART_PASSWORD="%[1]s" /apps/bin/stalwart-cli apply --file /configs/stalwart/core_config_plan.ndjson 2>&1 || echo "[bootstrap] Stalwart core config plan may have already been applied"`, cfg.StalwartAdminPassword))
		...
		cmds = append(cmds, `/apps/bin/stalwart --config /configs/stalwart/config.json >/var/log/stalwart.log 2>&1 &`)
...
if ! su git -s /bin/sh -c "FORGEJO_WORK_DIR=/data/forgejo FORGEJO_CUSTOM=/configs/forgejo/custom /apps/bin/forgejo --config /configs/forgejo.ini admin auth list" 2>/dev/null | grep -q "TinyAuth"; then
    echo "[bootstrap] Registering TinyAuth OIDC in Forgejo..."
    su git -s /bin/sh -c "FORGEJO_WORK_DIR=/data/forgejo FORGEJO_CUSTOM=/configs/forgejo/custom /apps/bin/forgejo --config /configs/forgejo.ini admin auth add-oauth \
    ...
    su git -s /bin/sh -c "FORGEJO_WORK_DIR=/data/forgejo FORGEJO_CUSTOM=/configs/forgejo/custom /apps/bin/forgejo --config /configs/forgejo.ini admin user create --admin --username %[4]s --password '%[5]s' --email %[4]s@%[6]s --must-change-password=false" || true
    
    # Restart forgejo to pick up the new auth source
    pkill -f "forgejo web" || true
    sleep 2
    su git -s /bin/sh -c "FORGEJO_WORK_DIR=/data/forgejo FORGEJO_CUSTOM=/configs/forgejo/custom /apps/bin/forgejo --config /configs/forgejo.ini web" >/var/log/forgejo.log 2>&1 &
fi
